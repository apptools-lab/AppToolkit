...
const UserGitConfigList: FC<{}> = () => {
  ...
  const onRemoveUserGitConfig = (configName: string) => {
    Dialog.confirm({
      title: '提示',
      content: `是否删除 ${configName} 配置？`,
      onOk: async () => {
        const res = await dispatcher.removeUserGitConfig({ configName, gitDir: '' });
        if (res) {
      ...
      </div>
      {
        !!userGitConfigs.length && (
          <Collapse defaultExpandedKeys={['0']}>
            {userGitConfigs.map((userGitConfig: IUserGitConfig, index: number) => {
              const { configName } = userGitConfig;
              const title = (
                <div className={styles.panelHeader}>
                  <div className={styles.title}>配置 - {configName}</div>
                  <Icon type="trash" className={styles.removeIcon} onClick={() => onRemoveUserGitConfig(configName)} />
                </div>
              );
              return (
